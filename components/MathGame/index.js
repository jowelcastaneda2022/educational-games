...
const MathGame = ({ roundsData, onFinish }) => {
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(
    Array(roundsData[0].questions.length).fill(null)
  );
  ...
  const handleNextRound = () => {
    ...
    if (currentRound < roundsData.length - 1) {
      ...
      setSelectedAnswers(
        Array(roundsData[currentRound + 1].questions.length).fill(null)
      );
    } else {
      ...
      <div className="questions">
        {roundsData[currentRound].questions.map((q, index) => (
          <div
            key={index}
            className={`question ${
              selectedAnswers[index] !== null ? "has-answer" : ""
            }`}
          >
